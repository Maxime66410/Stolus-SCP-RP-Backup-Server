...
function core.derma.createStaff(staff_list)
	...
	btn.Paint = function(self, w, h)
		self.lerp = Lerp(FrameTime()*6, self.lerp or 0, self.hover and 1 or 0);
		draw.SimpleText("Player list", "Raleway Bold 15", w/2, h/2, !staff_list and main_color or Color(255, 255, 255, 150 + 150*self.lerp), 1, 1);
	...
	btn.Paint = function(self, w, h)
		self.lerp = Lerp(FrameTime()*6, self.lerp or 0, self.hover and 1 or 0);
		draw.SimpleText("Staff online", "Raleway Bold 15", w/2, h/2, staff_list and main_color or Color(255, 255, 255, 150 + 150*self.lerp), 1, 1);
    ...
    if (core.derma.states.sortBy != "Names") then
	    for _, cat in pairs(categories) do
			...
			for k, v in pairs(player.GetAll()) do
				...
				if (RPExtraTeams[v:Team()].category != cat.name) then continue; end;
...
function core.derma.OpenProfile(ply)
	...
	header.Paint = function(self, w, h)
		draw.SimpleText("User profile", "Raleway Bold 15", 0, h/2, Color(255, 255, 255, 255), 0, 1);
	...
	header.Paint = function(self, w, h)
		draw.SimpleText(lply():Name(), "Raleway Bold 29", 0, 0, main_color, 0, 0);
		draw.SimpleText("Rank: "..(core.config["staff_replace"][lply():GetUserGroup()] and core.config["staff_replace"][lply():GetUserGroup()] or lply():GetUserGroup()), "Raleway 15", 0, 29, Color(255, 255, 255, 150), 0, 0);
	...
	header.Paint = function(self, w, h)
		draw.SimpleText("Main info", "Raleway Bold 17", 0, h/2, main_color, 0, 1);
	...
	kills.Paint = function(self, w, h)
		draw.SimpleText("Kills count", "Raleway Bold 15", 0, h/2, Color(255, 255, 255), 0, 1);
	...
	deaths.Paint = function(self, w, h)
		draw.SimpleText("Death count", "Raleway Bold 15", 0, h/2, Color(255, 255, 255), 0, 1);
	...
	deaths:SetSize(sizex, 15);
	deaths.Paint = function(self, w, h)
		draw.SimpleText("Ping", "Raleway Bold 15", 0, h/2, Color(255, 255, 255), 0, 1);
	...
	job.Paint = function(self, w, h)
		draw.SimpleText("Job", "Raleway Bold 15", 0, h/2, Color(255, 255, 255), 0, 1);
		draw.SimpleText(team.GetName(lply():Team()), "Raleway 15", col_size(), h/2, Color(255, 255, 255, 150), 0, 1);
	end;

	local ugroup = vgui.Create("DPanel", info_items);
	ugroup:SetSize(sizex, 15);
	ugroup.Paint = function(self, w, h)
		draw.SimpleText("Usergroup", "Raleway Bold 15", 0, h/2, Color(255, 255, 255), 0, 1);
		draw.SimpleText(lply():GetUserGroup(), "Raleway 15", col_size(), h/2, Color(255, 255, 255, 150), 0, 1);
	end;
	...
	header:SetSize(sizex,23);
	header.Paint = function(self, w, h)
		draw.SimpleText("Steam Profile", "Raleway Bold 17", 0, h/2, main_color, 0, 1);
		surface.SetFont("Raleway Bold 17");
	...
	steam_data.Paint = function(self, w, h)
		...
		draw.SimpleText("Steam Name:", "Raleway Bold 15", 0, h/2, Color(255, 255, 255), 0, 1);
	...
	steam_data.Paint = function(self, w, h)
		self.lerp = Lerp(FrameTime()*4, self.lerp or 0, self.hovered and 1 or 0);
		draw.SimpleText("Community link:", "Raleway Bold 15", 0, h/2, Color(255, 255, 255), 0, 1);
	...
	if (core.config["fadmin"]) then
		...
		header:SetSize(sizex,23);
		header.Paint = function(self, w, h)
			draw.SimpleText("FAdmin commands", "Raleway Bold 17", 0, h/2, main_color, 0, 1);
			surface.SetFont("Raleway Bold 17");
	    ...
	    for _, v in ipairs(FAdmin.ScoreBoard.Player.ActionButtons) do
	        if (v.Visible == true or (type(v.Visible) == "function" and v.Visible(lply()) == true)) then
	            local name = v.Name
	            if type(name) == "function" then name = name(lply()) end
